...
userRouter.post('/', async (ctx, next) => {
  if (ctx.state.userId) ctx.throw(403, 'Header에 있는 JWT를 제외하고 다시 요청해주세요.')

  const { email, passwordHash } = ctx.request.body

  if (!emailRegEx.test(email)) ctx.throw(400, '이메일 형식에 맞춰서 입력해주세요.')
  ...
  if (!userId) ctx.throw(400, '이미 존재하는 이메일입니다.')
...
userRouter.post('/login', async (ctx, next) => {
  const userId = ctx.state.userId
  if (userId) ctx.throw(403, 'Header에 있는 JWT를 제외하고 다시 요청해주세요.')
  ...
  if (rowCount === 0) ctx.throw(401, '로그인에 실패했어요. 이메일 또는 비밀번호를 확인해주세요.')
